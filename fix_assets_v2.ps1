# Fix asset paths in all view files - Version 2
...
foreach ($file in $viewFiles) {
    ...
    $content = $content -replace 'href="<?php echo base_url\(''assets/([^'']+)''\) ?>"', 'href="<?php echo base_url(''assets/$1'') ?>"'
    
    # Fix src="assets/ to src="<?php echo base_url('assets/') ?>"
    $content = $content -replace 'src="assets/', 'src="<?php echo base_url(''assets/'
    $content = $content -replace 'src="<?php echo base_url\(''assets/([^'']+)''\) ?>"', 'src="<?php echo base_url(''assets/$1'') ?>"'
    
    # Fix url(assets/ to url(<?php echo base_url('assets/') ?>)
    $content = $content -replace 'url\(assets/', 'url(<?php echo base_url(''assets/'
    $content = $content -replace 'url\(<?php echo base_url\(''assets/([^'']+)''\) ?>\)', 'url(<?php echo base_url(''assets/$1'') ?>)'
